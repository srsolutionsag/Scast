...
abstract class AR {
	...
	public function updateInto() {
		if(self::where(array(self::returnPrimaryFieldName() => self::getPrimaryFieldValue()))->hasSets()) {
			$this->update();
		} else {
			$this->create();
		}
	...
	public static function where($where, $operator = NULL) {
		$srModelObjectList = new ARList(get_called_class());
	 ...
	 * @throws Exception
	 */
	private static function setDBFields(AR $obj) {
